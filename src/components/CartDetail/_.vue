...
<template>
  <div v-show="store.showCart"
    class="w-screen md:(w-100 rounded-l-2xl) h-screen px-6 py-4 fixed top-0 right-0 flex flex-col z-20 bg-white">
    <!-- Close -->
    ...
    <div class="mt-2 flex-grow flex-shrink overflow-y-auto">
      <div class="h-15 my-3 border-3 p-3 flex justify-between items-center rounded-lg border-gray-300"
        ...
        <span class="flex flex-center">
          <PlusMinusImg @click="store.cart[x]--" :src="minus" />
          <span class="mx-1">{{ quantity }}</span>
          <PlusMinusImg @click="store.cart[x]++" :src="plus" />
        </span>
      </div>
    </div>
    <!-- Bottom Bar -->
