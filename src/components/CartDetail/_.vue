...
<template>
  <!-- Cart Details -->
  <div
    class='absolute top-0 h-screen flex z-19 overflow-hidden transition-all duration-1000 ease-in-out'
    :class="[store.showCart ? 'w-screen' : 'w-0', store.showCart ? 'left-0' : 'left-[100vw]']">
    <!-- Background shadow -->
    <div
      class='<md:hidden w-full h-full flex-grow bg-gray-400 bg-opacity-80                                                          0'
      @click='store.showCart = false'></div>
    <!-- Real Cart -->
    <div
      class='w-screen md:(basis-100 rounded-l-2xl) h-screen px-6 py-4 flex-grow-0 flex-shrink-0 flex flex-col z-20 bg-white'>
      <!-- Close -->
      <span class='h-13 flex-grow-0 flex-shrink-0 flex'>
        <button type='button' class='rounded-full shadow bg-gray-200 cursor-pointer' @click='store.showCart = false'>
      ...
      <div class='mt-2 flex-grow flex-shrink overflow-y-auto scroll-smooth'>
        <div
          v-for='[x, quantity] of store.cart' :key='x.cnName'
      ...
      <span class='h-13 px-4 mt-4 mb-1 flex-grow-0 flex-shrink-0 flex flex-center'>
        ...
        bg-gray-500 text-xl text-white' @click='store.cart.clear'>
